...
def main(args):
    ...
    id_to_caption = dict(zip(df['ID'], df['caption']))
    ...
    for video_ids, f_idxs, frames in tqdm(loader):
        ...
        vids = [int(video_id.split("/")[-1]) for video_id in video_ids]

        captions = []
        for vid in vids:
            caption = id_to_caption.get(vid, f"video caption")
            captions.extend([caption] * nf)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--video_dir", type=Path, default="datasets/WebVid/2M/")
