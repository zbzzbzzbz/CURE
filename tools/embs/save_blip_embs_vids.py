        ...
        for video_id, f_idx, frm_feat in zip(video_ids, f_idxs, frm_feats):
            ...
            if args.save_all_tokens:
                if args.method == 'middle':
                    frm_feat = frm_feat[len(frm_feat) // 2]
                elif args.method == 'mean':
                    frm_feat = frm_feat.mean(0)
                else:
                    raise ValueError(f"method wrong: must be middle or mean")
...
if __name__ == "__main__":
    ...
    if args.save_all_tokens:
        print(f"precomputing ref img  !!!\n")
    else:
        print(f"precomputing tar img  !!!\n")
    main(args)
