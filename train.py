...
from utility import get_correspondence
...
def main(cfg: DictConfig):
    ...
    for epoch in range(cfg.trainer.max_epochs):
        ...
        if epoch < cfg.warmup_epochs:
            fabric.print(f"warmup starting : {epoch + 1}/{cfg.warmup_epochs}")
            warmup(model, loader_train, optimizer, fabric, cfg)
        else:
            fabric.print(f"train starting")

            preds, norm_es_eye, uncertainty = get_correspondence(model, loader_train, epoch)
            fabric.print('the proportion of clean samples are ', preds.sum().item() / preds.shape[0])
            train(model, loader_train, optimizer, fabric, epoch, cfg, uncertainty, preds=preds)
...
def warmup(model, train_loader, optimizer, fabric, cfg):
    model.train()
    for batch_idx, batch in enumerate(train_loader): 
        optimizer.zero_grad()
        loss = model.warmup_batch(batch, fabric)
...
def train(model, train_loader, optimizer, fabric, epoch, cfg, uncertainty, preds):
    model.train()

    for batch_idx, batch in enumerate(train_loader):        
        optimizer.zero_grad()
        
        preds_cur = preds.to(model.device)[batch['pair_id']]
        uncertainty_cur = uncertainty.to(model.device)[batch['pair_id']]
        loss = model(batch, fabric, preds_cur, uncertainty_cur)
