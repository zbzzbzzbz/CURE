...
class VideoDataset(Dataset):
    def __init__(
        ...
        if isinstance(todo_ids, (str, Path)):
            todo_ids = get_ids(todo_ids, save_all_tokens)
...
def get_ids(path, save_all_tokens=False):
    suffix = Path(path).suffix
    if suffix == ".csv":
        df = pd.read_csv(path)
        if save_all_tokens:
            ids = set(df["pth1"].tolist())
        else:
...
import csv
import json
class FineCVRVideoDataset(Dataset):
    def __init__(
        self,
        video_dir,
        dataset_root,
        ...
        if split == 'train':
            self.dataset = pd.read_table(self.dataset_root / 'train_remaped_unique_with_captions.txt', names=['idx', 'ref', 'target', 'cap','source','class_id','ref_caption','target_caption'], header=None, quoting=csv.QUOTE_NONE)
            id2vdoname_pt = os.path.join(self.dataset_root, "id2vdoname_train.json")
        elif split == 'test':
            self.dataset = pd.read_table(self.dataset_root / 'test_remaped_unique_with_captions.txt', names=['idx', 'ref', 'target', 'cap','source','class_id','ref_caption','target_caption'], header=None, quoting=csv.QUOTE_NONE)
            id2vdoname_pt = os.path.join(self.dataset_root, "id2vdoname_test.json")
        else:
        ...
        todo_ids = self.dataset['target'].tolist()
        print(f"len(todo_ids) : {len(todo_ids)}")

        target_caption_pairs = self.dataset[['target', 'target_caption']].drop_duplicates(subset=['target'])
        self.id2target_caption = {}
        for _, row in target_caption_pairs.iterrows():
            target_id = str(row['target'])  
            video_name = self.id2vdoname[target_id]  
            video_id = self.name_topath(video_name).name 
            self.id2target_caption[video_id] = row['target_caption']

        found_paths = list([p for p in video_dir.glob("**/*") if p.is_dir() and len(p.relative_to(video_dir).parts) == 2])
        if todo_ids is not None:
            video_names = [self.id2vdoname[str(id)] for id in todo_ids]
            video_paths = [self.name_topath(name) for name in video_names]
            video_paths = list(set(video_paths) & set(found_paths))
        else:
            video_paths = found_paths
            
        video_paths.sort()
        self.id2path = {pth.name: pth for pth in video_paths}
        self.video_ids = list(self.id2path.keys())
        self.video_ids.sort()
    ...
    def __getitem__(self, index):
        video_id = self.video_ids[index]
        video_path = self.id2path[video_id]
        frames, f_idxs = get_video_frames_finecvr(
            video_path, self.frames_video, self.image_size
        )
        frames = [self.transform(frame) for frame in frames]
        frames = torch.stack(frames, dim=0)
        ...
        target_caption = self.id2target_caption[video_id]

        return video_id, f_idxs, frames, target_caption



    def name_topath(self, vdo_name):
        if 'video' in vdo_name:
            frames_path = self.video_dir / "msrvtt_frames"
        elif len(vdo_name)==5:
            frames_path = self.video_dir / "ag_frames"
        elif len(vdo_name)>13 and len(vdo_name)<17:
            frames_path = self.video_dir / "an_frames"
        else:
            frames_path = self.video_dir / "hvu_frames"
            if 'hvu' in str(frames_path):
                ...
                if os.path.exists(os.path.join(frames_path, vdo_name)):
                    pass
                else:
                    raise ValueError("Path miss!!")
                

        video_path = frames_path / vdo_name
        return video_path
    
    def name_toparentdir(self, vdo_name):
        if 'video' in vdo_name:
            parent_dir = "msrvtt_frames"
        elif len(vdo_name)==5:
            frames_path = "ag_frames"
        elif len(vdo_name)>13 and len(vdo_name)<17:
            frames_path = "an_frames"
        else:
            frames_path = "hvu_frames"
        return frames_path
    

def get_video_frames_finecvr(video_pth, frames_video=8, image_size=384):
    import cv2

    video_pth = str(video_pth)
    
    if not os.path.isdir(video_pth):
        frames = [
            Image.fromarray(np.zeros((image_size, image_size, 3), dtype=np.uint8))
        ] * frames_video
        f_idxs = [-1] * frames_video
        return frames, f_idxs

    jpg_files = sorted([
        f for f in os.listdir(video_pth) 
        if f.lower().endswith('.jpg') and os.path.isfile(os.path.join(video_pth, f))
    ])
    ...
    for i, file in enumerate(jpg_files):
        file_path = os.path.join(video_pth, file)
    ...
    if n_frames < frames_video:
        if n_frames == 0:
            last_frame = Image.fromarray(np.zeros((image_size, image_size, 3), dtype=np.uint8))
        else:
            last_frame = frames[-1]
        
        need = frames_video - n_frames
        frames += [last_frame] * need
        f_idxs += [f_idxs[-1]] * need if f_idxs else [-1] * need
    
    if len(frames) > frames_video:
        frames = frames[:frames_video]
        f_idxs = f_idxs[:frames_video]
...
class FineCVRTextDataset(Dataset):
    def __init__(
        self,
        video_dir,
        dataset_root,
    ...
    def __getitem__(self, index):
        ...
        return video_id, target_caption
