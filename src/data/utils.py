...
class FineCVR_FrameLoader:
    def __init__(self, transform, frames_video=1, method="middle", image_size=384):
        self.transform = transform
        self.method = method
        self.frames_video = frames_video
    ...
    def __call__(self, video_pth: str):
        ...
        if not os.path.isdir(video_pth):
            frames = [
                Image.fromarray(np.zeros((self.image_size, self.image_size, 3), dtype=np.uint8))
            ] * self.frames_video
            video_data = [self.transform(frame) for frame in frames]
            return video_data
        ...
        video_datas = [self.transform(frame) for frame in frames]
        if self.method == 'middle':
            video_data = video_datas[len(video_datas) // 2]
        elif self.method == 'mean':
            video_data = video_datas.mean(0)
        else:
            raise ValueError(f"self.method must be mean or middle")
